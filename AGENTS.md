...
`@unif/react-native-umeng` —— 友盟 RN 新架构桥,做两件事:**U-Share**(微信会话 + 钉钉分享)与 **U-App 移动统计**。当前仓库开发/example 验证基线：RN 0.86.2、React 19.2.3、Design 0.21.1。Design 的 peer 与 dev 同为 `^0.21.1`——本仓已按 0.21 收紧后的 `Cell.leading` / `TextFieldSlot` 契约实现，0.20 编译不过，不在支持范围。TypeScript 6。UI 文案中文。首版只支持微信会话 + 钉钉(无朋友圈 / QQ / 微博)。
