...
- 钉钉分享
- 分享内容类型：文本 / 图片 / 链接卡片
- 友盟移动统计：`UMConfigure.preInit` + `UMConfigure.init` 两阶段、`onEvent`、`reportError`、`onProfile signIn/signOut`
...
采用方案 B —— **拆两个独立 TurboModule**：

```
src/
├── index.ts                 ← re-export Share / Analytics / 类型
...
ios/
├── UmengShare.swift         ← @objc class, 实现 UmengShareSpec 协议
├── UmengAnalytics.swift     ← @objc class, 实现 UmengAnalyticsSpec 协议
├── UmengBootstrap.swift     ← 共享的 preInit + setPlatform 单例
└── ReactNativeUmeng-Bridging-Header.h  ← 引入 <UMCommon/...>、<UMShare/...> 头

android/src/main/java/com/unif/reactnativeumeng/
├── UmengShareModule.kt           ← TurboModule 1
├── UmengAnalyticsModule.kt       ← TurboModule 2
├── UmengBootstrap.kt             ← 共享的 preInit + setPlatform 单例
└── ReactNativeUmengPackage.kt    ← 注册两个 TurboModule

ReactNativeUmeng.podspec
```

**两个 TurboModule 通过 `UmengBootstrap` 单例共享一次性 preInit**：
- App 启动时，模块构造器（iOS `init()`、Android `initialize()`）先调用 `UmengBootstrap.shared.ensurePreInit(context)`
- `UmengBootstrap` 内部加锁，保证 preInit + setPlatform 仅执行一次
...
```ts
// src/analytics.ts
export function init(): Promise<void>;                       // 用户同意隐私协议后调
...
JS facade（`share.ts` / `analytics.ts`）负责把 Platform enum 转字符串、组织参数、把原生回包翻成 ShareResult 或抛错。
...
JS 启动
   │
   ▼ 用户首次同意《隐私协议》
JS: await Analytics.init()
   │
   ▼ 原生：UMConfigure.init(appkey, channel, deviceType, secret)
开始正式数据采集 + 分享回流统计
```
...
| `E_NOT_INITIALIZED` | `Analytics` API 在 init 前调用 | `UmengBootstrap.isInited` 标志 |
...
2. JS 侧：types / NativeUmengShare / NativeUmengAnalytics / share / analytics / index + 单测
3. Android：`UmengBootstrap` + `UmengShareModule` + `UmengAnalyticsModule` + `ReactNativeUmengPackage`，gradle 依赖
4. iOS：`UmengBootstrap.swift` + `UmengShare.swift` + `UmengAnalytics.swift`，bridging header，podspec 依赖
5. example：9 分享按钮 + 5 统计按钮
