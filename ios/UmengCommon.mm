...
- (void)preInit:(NSDictionary *)config resolve:(RCTPromiseResolveBlock)resolve reject:(RCTPromiseRejectBlock)reject {
  NSError *error = nil;
  BOOL ok = [[UmengBootstrap shared] ensurePreInit:config error:&error];
  if (ok) {
    resolve([NSNull null]);
  } else {
    reject(@"E_INVALID_OPTIONS", error.localizedDescription ?: @"preInit failed", error);
  }
}
...
- (void)init:(RCTPromiseResolveBlock)resolve reject:(RCTPromiseRejectBlock)reject {
  ...
  if (ok) {
    ...
    reject(code, error.localizedDescription ?: @"init failed", error);
  }
}
