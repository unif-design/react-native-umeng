...
**只用 yarn** —— 项目依赖 yarn workspaces(`packageManager: yarn@4.11.0`、`nodeLinker: node-modules`、`nmHoistingLimits: workspaces`)。pre-commit hook(lefthook)对 staged 文件跑 `eslint` + `tsc`,native 仓还跑 `clang-format`(`.mm`)/ `ktlint`(`.kt`);commit-msg hook 用 commitlint 强校验 conventional commits。`lefthook.yml` 由 org 模板同步,不要单仓 drift。
