...
test('usage docs link native setup and distinguish device validation from automation', async () => {
  const readme = await readFile(
    new URL('../../README.md', import.meta.url),
    'utf8'
  );
  assert.match(readme, /website\/docs\/native-setup\/ios\.md/);
  assert.match(readme, /website\/docs\/native-setup\/android\.md/);
  assert.match(readme, /真实分享[\s\S]*设备/);
  const integration = await readFile(
    new URL('../../example/INTEGRATION.md', import.meta.url),
    'utf8'
  );
  assert.match(integration, /真实[\s\S]*回包[\s\S]*真机/);
  const share = await readFile(
    new URL('../../website/docs/api/share.md', import.meta.url),
    'utf8'
  );
  assert.match(share, /不能替代[\s\S]*真实 App/);
});
